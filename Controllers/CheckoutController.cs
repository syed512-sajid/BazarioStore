...
namespace EcommerceStore.Controllers
{
    public class CheckoutController : Controller
    {
        ...
        public CheckoutController(ApplicationDbContext context, ILogger<CheckoutController> logger, IConfiguration configuration)
        {
            _context = context;
            _logger = logger;
        ...
        private void SendAdminEmail(Order order, List<CartItem> cart)
        {
            ...
            string body = $"<h2>New order #{order.Id} received from {order.CustomerName}</h2>";
            foreach (var item in cart)
            {
                body += $"<p>{item.ProductName} - Qty: {item.Quantity} - Price: {item.Price:N0}</p>";
            }
